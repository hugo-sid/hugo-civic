...
- **🌐 Multilingual.** English, Spanish, Hindi, Odia, Tamil and Telugu translations included; all interface strings go through `i18n`.
...
- **Banner and identifier wording is federally standardised — where a source exists.** The English and Spanish strings are copied verbatim from USWDS; do not paraphrase them. USWDS publishes no official Hindi, Odia, Tamil or Telugu translation of this wording, so `i18n/hi.toml`'s, `i18n/or.toml`'s, `i18n/ta.toml`'s and `i18n/te.toml`'s banner/identifier strings — and the rest of their UI strings — were translated for this repository and have had no native-speaker review. See HINDI.md, ODIA.md, TAMIL.md and TELUGU.md.
- **A script outside USWDS's fonts needs its own font.** Public Sans, Merriweather and Roboto Mono are Latin-only, so Hindi is set in a self-hosted Noto Sans Devanagari, Odia in Noto Sans Oriya, Tamil in Mukta Malar and Telugu in Noto Sans Telugu (all `@fontsource`), each scoped to its own `:lang()` and published only on pages in that language. A face that sets smaller than Public Sans can carry a `font-size-adjust` alongside it, as Tamil's does — see TAMIL.md §3; Telugu measured close enough to Public Sans not to need one, TELUGU.md §3. Pick the face on measurement rather than on family consistency: TELUGU.md §2 records the stroke-contrast test that chose a sans over the serif of the same superfamily. Adding a script is a table entry plus an `@font-face` block, not a new template:
...
- **Pagefind stems some languages and not others.** It names the ones it cannot on every index run — here, `or-in` and `te-in` — and those search without matching across root forms. Read that output rather than assuming from the script: `hi-in` and `ta-in` are stemmed, `or-in` and `te-in` are not, and no property of the script predicts which.
